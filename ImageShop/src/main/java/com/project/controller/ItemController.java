...
public class ItemController {
	...
	public ResponseEntity<byte[]> displayFile(Item item) throws Exception {
		...
		// 썸네일이미지파일명을 DB로부터 가져온다
		// bd6b87d-458d-4738-bf3c-75229c527a3d_kdj2.jpg
		String fileName = itemService.getPreview(item);
		try {
			// bd6b87d-458d-4738-bf3c-75229c527a3d_kdj2.jpg => jpg 확장자명을 가져온다.
			...
			// D:/upload/bd6b87d-458d-4738-bf3c-75229c527a3d_kdj2.jpg 위치를 찾아서 파일을 InputStream으로 읽는다.
			// ( / <- File.separator )
			// InputStream을 통해 파일을 바이트단위로 읽는다.
			in = new FileInputStream(uploadPath + File.separator + fileName);

			if (mType != null) {
				// httpHearder는 서버가 브라우저엑 ㅔ정보를 담아서 보내주는 객체에 미디어타입을 세팅한다.
				headers.setContentType(mType);
			}
			
			// Json방식으로 IOUtils.toByteArray(in)을 전송(써있는대로 byte단위)
		...
		// Webbrowser에게 byte단위로 이미지를 전송.
		return entity;
	}
	
	@ResponseBody
	@RequestMapping("/picture")
	// modify  picture/preview의 ${item.itemId}를 (Item item)으로 받아옴
